...
public class ResultPage //using ObjectRepo
{
	...
	{
		int expectedCount= Integer.parseInt(objr.findEle("product_count").getText());
	...
	public  int[] resultItemCount(WebDriver driver) throws InterruptedException
	{
		...
		while(newPage)
		{
			...
			List<WebElement> pageItems= wait.until(ExpectedConditions.visibilityOfAllElementsLocatedBy(By.cssSelector(".shortDescription")));
			...
			try
			{
				wait.until(ExpectedConditions.visibilityOf(objr.findEle("next_page_arrow"))).click();
			}
			catch (Exception e) 
			{	newPage=false;	
			}
	...
	{
		WebDriverWait wait=new WebDriverWait(driver, 30);
		wait.until(ExpectedConditions.visibilityOf(objr.findEle("click_on_item"))).click();
